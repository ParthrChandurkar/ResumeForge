...
export const api = {
  ...
  revise: (id, instruction, target) => request(`/tailor/${id}/revise`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ instruction, target }) }),
  companyResearch: (id) => request(`/tailor/${id}/company-research`, { method: 'POST' }),
  shortMessage: (id) => request(`/tailor/${id}/short-message`, { method: 'POST' }),
