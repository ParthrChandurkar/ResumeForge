function Contact({contact={},variant}) {
  ...
  return <div className="doc-contact">{contact.phone&&<a href={phoneHref}>☎ {contact.phone}</a>}{contact.email&&<a href={emailHref}>✉ {contact.email}</a>}{cloud&&contact.github&&<a href={contact.github} target="_blank" rel="noreferrer">◉ GitHub</a>}{contact.linkedin&&<a href={contact.linkedin} target="_blank" rel="noreferrer">in LinkedIn</a>}{contact.portfolio&&<a href={contact.portfolio} target="_blank" rel="noreferrer">◉ Portfolio</a>}</div>
}

export function ResumePreview({ result, printCurrent=false }) {
  const r=result.resume, consulting=result.template_track==='consulting'
  return <article className={`paper resume-paper ${consulting?'consulting-paper':'cloud-paper'} ${printCurrent?'print-current':''}`}>
    ...
    <DocSection title={r.skills_title} compact>{r.skill_groups?.map((group,i)=><p key={i}><strong>{group.label}:</strong> {group.items}</p>)}</DocSection>
    <DocSection title={r.experience_title}>{r.experiences?.map((entry,i)=><DocEntry entry={entry} key={i}/>)}</DocSection>
    {!!r.secondary_entries?.length&&<DocSection title={r.secondary_title}>{r.secondary_entries.map((entry,i)=><DocEntry entry={entry} key={i}/>)}</DocSection>}
...
function DocEntry({entry}){return <div className="doc-entry"><div className="entry-head"><strong>{entry.title}</strong><strong>{entry.date}</strong></div><div className="entry-sub"><em>{linkEntry(entry.subtitle,entry.url)}</em><em>{entry.location}</em></div>{entry.technologies&&<div className="technologies"><em>{entry.technologies}</em></div>}<ul>{entry.bullets?.map((bullet,i)=><li key={i}>{bullet}</li>)}</ul></div>}
